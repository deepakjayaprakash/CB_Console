	...
	if($exists > 0) //IF there are no returning rows or no existing username
	{
		...
		if(($username == $table_users) && ($password == $table_password)) // checks if there are any matching fields
		{
				if($password == $table_password)
				{
							...
							Print '<script>
							window.location.assign("http://newstag.cloudbyte.com/partner-dashboard/");</script>'; 
			...
			Print '<script>window.location.assign("http://newstag.cloudbyte.com/partner-page-login/");</script>'; // redirects to login.php
		}
    }
else{
    Print '<script>alert("Incorrect Information!");</script>'; //Prompts the user
